...
use crate::types::{Mode, OutputEncoding, Padding};
...
pub fn encrypt_cbc(data: &[u8], key: &[u8], iv: &[u8]) -> Result<Vec<u8>> {
    if key.len() != 8 {
        return Err(CryptoError::InvalidArgument(
            "DES requires 8-byte key".to_string(),
        ));
    }

    if iv.len() != 8 {
        return Err(CryptoError::InvalidArgument(
            "DES requires 8-byte IV".to_string(),
        ));
    }
    ...
    match result {
        ...
        Err(e) => Err(CryptoError::InvalidArgument(format!(
            "Encryption failed: {e:?}"
        ))),
    }
}
...
pub fn decrypt_cbc(ciphertext: &[u8], key: &[u8], iv: &[u8]) -> Result<Vec<u8>> {
    ...
    let plaintext = cipher
        ...
        .map_err(|_| {
            CryptoError::Message("Decryption failed - invalid ciphertext or key".to_string())
        })?;
...
pub fn encrypt(
    ...
    match mode {
        ...
        _ => Err(CryptoError::InvalidArgument(format!(
            "DES mode {mode:?} not yet implemented"
        ))),
    }
}
...
pub fn encrypt_file_streaming(
    ...
    match mode {
        ...
        _ => Err(CryptoError::InvalidArgument(format!(
            "Streaming not supported for mode {mode:?}"
        ))),
    }
}
