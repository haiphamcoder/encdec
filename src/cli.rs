...
use crate::util::{
    base64_decode, base64_encode, hex_encode, read_input, write_output, InputSource, OutputTarget,
};
...
pub struct CryptoArgs {
    ...
    #[arg(
        long = "key",
        help = "Provide encryption key directly as a string. 
    ...
    #[arg(
        long = "key-file",
        help = "Load encryption key from a file. 
    ...
    #[arg(
        long = "stream",
        help = "Enable streaming I/O for processing large files efficiently. 
...
fn handle_keygen(args: KeygenArgs) -> Result<()> {
    match args.algorithm {
        Algorithm::Aes => {
            ...
            println!(
                "AES-{} key ({:?}): {}",
                args.size, args.output_encoding, formatted
            );
        }
        Algorithm::Des => {
        ...
        Algorithm::Rsa => {
            ...
            if args.private_out.is_none() && args.public_out.is_none() {
                println!(
                    "RSA-{} key pair generated (use --private-out and --public-out to save)",
                    args.size
                );
            }
        }
...
fn handle_encrypt(args: CryptoArgs) -> Result<()> {
    ...
    let key = if args.algorithm == Algorithm::Rsa {
        ...
        return Err(crate::error::CryptoError::InvalidArgument(
            ...
            For RSA encryption, use --public-key instead."
                .to_string(),
        ));
    };
    ...
    let output = match args.algorithm {
        ...
        Algorithm::Rsa => {
            ...
            let public_key = if let Some(key_path) = &args.public_key {
                rsa::load_public_key_pem(key_path).map_err(|e| {
                    crate::error::CryptoError::InvalidArgument(format!(
                        "Failed to load RSA public key from '{key_path}': {e}"
                    ))
                })?
            } else {
                return Err(crate::error::CryptoError::InvalidArgument(
...
fn handle_encrypt_streaming(args: CryptoArgs) -> Result<()> {
    ...
    let key = if args.algorithm == Algorithm::Rsa {
        return Err(CryptoError::InvalidArgument(
            "Streaming not supported for RSA encryption".to_string(),
        ));
    } else if let Some(key_str) = &args.key {
        load_key_from_string(key_str, args.algorithm)?
    } else if let Some(key_file) = &args.key_file {
        ...
        return Err(CryptoError::InvalidArgument(
            "Must provide either --key or --key-file".to_string(),
        ));
    };
...
fn handle_decrypt_streaming(args: CryptoArgs) -> Result<()> {
    ...
    let key = if args.algorithm == Algorithm::Rsa {
        return Err(CryptoError::InvalidArgument(
            "Streaming not supported for RSA decryption".to_string(),
        ));
    } else if let Some(key_str) = &args.key {
        load_key_from_string(key_str, args.algorithm)?
    } else if let Some(key_file) = &args.key_file {
...
fn handle_decrypt(args: CryptoArgs) -> Result<()> {
    ...
    let data = if args.algorithm == Algorithm::Rsa {
        ...
        if let Some(data) = &args.input_data {
            ...
            let file_str = String::from_utf8(file_data).map_err(|_| {
                CryptoError::InvalidArgument("Invalid file encoding for RSA decryption".to_string())
            })?;
    ...
    let plaintext = match args.algorithm {
        Algorithm::Aes | Algorithm::Des => {
            ...
            let (iv_or_nonce, ciphertext) = match args.algorithm {
                Algorithm::Aes => {
                    let iv_len = match args.mode {
                        ...
                        _ => {
                            return Err(crate::error::CryptoError::InvalidArgument(
                                "Unsupported mode for decryption".to_string(),
                            ))
                        }
                    };
                    if data.len() < iv_len {
            ...
            match args.algorithm {
                Algorithm::Aes => {
                    aes::decrypt(&ciphertext, &key, args.mode, args.padding, &iv_or_nonce)?
                }
                Algorithm::Des => {
                    des::decrypt(&ciphertext, &key, args.mode, args.padding, &iv_or_nonce)?
                }
                _ => unreachable!(),
            }
        }
        Algorithm::Rsa => {
            ...
            let private_key = if let Some(key_path) = &args.private_key {
                ...
                return Err(crate::error::CryptoError::InvalidArgument(
                    "RSA decryption requires --private-key".to_string(),
                ));
            };
...
fn handle_sign(args: SignArgs) -> Result<()> {
    if args.algorithm != Algorithm::Rsa {
        return Err(CryptoError::InvalidArgument(
            "Sign currently supports RSA only".to_string(),
        ));
    }
    ...
    println!(
        "File signed successfully. Signature saved to: {}",
        args.output_sig
    );
    Ok(())
}
...
fn handle_verify(args: VerifyArgs) -> Result<()> {
    if args.algorithm != Algorithm::Rsa {
        return Err(CryptoError::InvalidArgument(
            "Verify currently supports RSA only".to_string(),
        ));
    }
...
fn validate_input_source(input_data: &Option<String>, input_file: &Option<String>) -> Result<()> {
    if input_data.is_some() && input_file.is_some() {
        return Err(crate::error::CryptoError::Message(
            "Provide either --input-data or --input-file, not both".to_string(),
        ));
    }
    if input_data.is_none() && input_file.is_none() {
        return Err(crate::error::CryptoError::Message(
            "Must provide one of --input-data or --input-file".to_string(),
        ));
    }
    Ok(())
}
...
fn load_key_from_string(key_str: &str, algorithm: Algorithm) -> Result<Vec<u8>> {
    ...
    match algorithm {
        ...
        Algorithm::Rsa => Err(crate::error::CryptoError::InvalidArgument(
            "RSA keys should be loaded from PEM files".to_string(),
        )),
    }
}
    ...
    match algorithm {
        Algorithm::Aes => {
            if key_data.len() == 32 {
                ...
                Err(crate::error::CryptoError::InvalidArgument(
                    "AES-256 requires 32-byte key file".to_string(),
                ))
            }
        }
        Algorithm::Des => {
            if key_data.len() == 8 {
                ...
                Err(crate::error::CryptoError::InvalidArgument(
                    "DES requires 8-byte key file".to_string(),
                ))
            }
        }
        Algorithm::Rsa => Err(crate::error::CryptoError::InvalidArgument(
            "RSA keys should be loaded from PEM files using --private-key or --public-key"
                .to_string(),
        )),
    }
}
