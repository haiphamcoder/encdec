...
fn test_encrypt_decrypt_aes_workflow() {
    ...
    let key_line = key_stdout.lines().find(|line| line.starts_with("AES-256 key (Base64):")).unwrap();
    ...
    let decrypted_data = fs::read_to_string("test_decrypted.txt").expect("Failed to read decrypted file");
...
fn test_encrypt_decrypt_rsa_workflow() {
    ...
    // Verify decrypted content
    let decrypted_data = fs::read_to_string("test_rsa_decrypted.txt").expect("Failed to read decrypted file");
    assert_eq!(decrypted_data, test_data);
...
fn test_streaming_encryption_workflow() {
    ...
    // Verify decrypted content
    let decrypted_data = fs::read_to_string("test_streaming_decrypted.txt").expect("Failed to read decrypted file");
    assert_eq!(decrypted_data, test_data);
